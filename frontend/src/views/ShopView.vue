...
const getProductImages = async (product: Product) => {
  try {
    const allImages = await api.get<string[]>('/gallery-images')
...
// products load asynchronously now; load their images once they arrive
watch(products, loadProductImages, { once: true })
