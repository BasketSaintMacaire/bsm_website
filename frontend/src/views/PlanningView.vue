...
// The API stores match dates as real UTC-midnight dates; convert back to the
// "dd/mm/yyyy" display string this view's filtering/grouping logic is built around.
...
onMounted(async () => {
  try {
    ...
    planningData.value = matches.map((m) => ({ ...m, date: isoToDdMmYyyy(m.date) }))
  } catch (e) {
