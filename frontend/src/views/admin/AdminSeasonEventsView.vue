...
const emptyForm = () => ({ name: '', day: '', month: '', year: '' })
const form = ref(emptyForm())

async function load() {
  loading.value = true
  try {
    items.value = await api.get<SeasonEvent[]>('/season-events')
  } finally {
    loading.value = false
  }
}
...
function openEdit(item: SeasonEvent) {
  form.value = { name: item.name, day: item.day, month: item.month, year: item.year }
...
async function save() {
  ...
  try {
    if (editingId.value !== null) {
      await api.put(`/season-events/${editingId.value}`, form.value)
    } else {
      await api.post('/season-events', form.value)
    }
    closeModal()
    await load()
  } finally {
...
async function remove(id: number) {
  ...
  await api.delete(`/season-events/${id}`)
  await load()
}
...
function triggerImport() {
  ...
  input.onchange = async (e) => {
    ...
    for (const row of rows) {
      try {
        await api.post('/season-events', { name: row.name, day: row.day, month: row.month, year: row.year })
        ok++
      } catch { /* skip */ }
    }
...
function downloadTemplate() {
  downloadCSVTemplate(
    'season_events_template.csv',
    ['name', 'day', 'month', 'year'],
    ['Tournoi de Noël', '20', 'Décembre', '2025'],
  )
}
</script>
      ...
      <table v-if="!loading" class="w-full text-sm">
        ...
        <tbody class="divide-y divide-gray-100">
          <tr v-for="item in items" :key="item.id" class="hover:bg-gray-50">
            ...
            <td class="px-4 py-3 text-gray-600">{{ item.day }}</td>
            <td class="px-4 py-3 text-gray-600">{{ item.month }}</td>
            <td class="px-4 py-3 text-gray-600">{{ item.year }}</td>
          ...
          <form @submit.prevent="save" class="space-y-3">
            ...
            <div class="grid grid-cols-3 gap-2">
              <div>
                ...
                <input v-model="form.day" required class="w-full border rounded px-3 py-1.5 text-sm" />
              </div>
              <div>
                <label class="block text-xs font-medium text-gray-600 mb-1">Mois</label>
                <input v-model="form.month" required class="w-full border rounded px-3 py-1.5 text-sm" />
              </div>
              <div>
                <label class="block text-xs font-medium text-gray-600 mb-1">Année</label>
                <input v-model="form.year" required class="w-full border rounded px-3 py-1.5 text-sm" />
              </div>
            </div>
