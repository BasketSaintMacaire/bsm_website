...
const emptyForm = () => ({ year: new Date().getFullYear(), title: '', description: '' })
const form = ref(emptyForm())

async function load() {
  loading.value = true
  try {
    items.value = await api.get<HistoryEvent[]>('/history-events')
  } finally {
    loading.value = false
  }
}
...
function openEdit(item: HistoryEvent) {
  form.value = { year: item.year, title: item.title, description: item.description }
...
async function save() {
  saving.value = true
  try {
    const payload = { year: Number(form.value.year), title: form.value.title, description: form.value.description }
    if (editingId.value !== null) {
      await api.put(`/history-events/${editingId.value}`, payload)
    } else {
      await api.post('/history-events', payload)
    }
    closeModal()
    await load()
  } finally {
...
async function remove(id: number) {
  ...
  await api.delete(`/history-events/${id}`)
  await load()
}
...
function triggerImport() {
  ...
  input.onchange = async (e) => {
    ...
    for (const row of rows) {
      try {
        await api.post('/history-events', { year: Number(row.year), title: row.title, description: row.description })
        ok++
      } catch { /* skip */ }
    }
...
function downloadTemplate() {
  downloadCSVTemplate(
    'history_events_template.csv',
    ['year', 'title', 'description'],
    ['1985', 'Fondation du club', 'Le BSM est fondé par des passionnés de basket.'],
  )
}
</script>
      ...
      <table v-if="!loading" class="w-full text-sm">
        ...
        <tbody class="divide-y divide-gray-100">
          <tr v-for="item in items" :key="item.id" class="hover:bg-gray-50">
            ...
            <td class="px-4 py-3 font-medium text-gray-800">{{ item.year }}</td>
        ...
        <div class="bg-white rounded-xl shadow-xl w-full max-w-lg p-6">
          ...
          <form @submit.prevent="save" class="space-y-3">
            <div>
              ...
              <input v-model.number="form.year" type="number" required class="w-full border rounded px-3 py-1.5 text-sm" />
            </div>
            <div>
            ...
            <div>
              ...
              <textarea v-model="form.description" rows="3" required class="w-full border rounded px-3 py-1.5 text-sm" />
            </div>
